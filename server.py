...
class Server():
    ...
    def collectData(self):
        ...
        if self.num_intervals >= 1:
            print("num connections in last interval",
                  self.num_connects_last_interval)
            self.avg_connects_per_interval = ((self.avg_connects_per_interval * (
                self.num_intervals-1)) + self.num_connects_last_interval) / self.num_intervals
            print("avg connections per interval",
                  self.avg_connects_per_interval)
            errorBound = self.avg_connects_per_interval * \
                self.marginOfError(self.num_intervals, 1.96)  # 95% conf level
    ...
    def acceptConnections(self):
        ...
        # strip HTTP request "GET /%s HTTP/1.1\r\n" to get filename
